...
module oscillator_cc058cf0 (SIMPV,IP,CELREF,fault_oscillator,ok_oscillator,osc,global_oscillator,CELSENSE_RF,CELFORCE_RF,IOSC,enable_oscillator,CELG,celkelvin_GNDoscillator,CELSUB);
  input  IP;
  output  osc;
  input  CELG;
  output  IOSC;
...
oscillator_cc058cf0 XIXoscext (
.IP(IP_c6de264d_XIXoscext),
.osc(osc),
.CELG(CELG59462),
.IOSC(IOSC),
.SIMPV(SIMPV),
...
.CELSUB(CELSUB40948),
.CELFORCE_RF(RT),
.CELSENSE_RF(TBD_Xoscillator_external1_XIXoscext_CELSENSE_RF),
.ok_oscillator(ok_oscillator),
.fault_oscillator(fault_oscillator),
.enable_oscillator(enable_oscillator),
.global_oscillator(tl0),
.celkelvin_GNDoscillator(TBD_Xoscillator_external1_XIXoscext_celkelvin_GNDoscillator)
);
